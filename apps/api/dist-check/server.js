...
var getVerifier = () => {
  ...
  if (!config2.cognitoUserPoolId || !config2.cognitoClientId) {
    return null;
  }
  if (!cognitoVerifier) {
    cognitoVerifier = CognitoJwtVerifier.create({
      userPoolId: config2.cognitoUserPoolId,
      clientId: config2.cognitoClientId,
      tokenUse: "id"
    });
  }
...
var summarizeDashboard = async (overview, rawFilters, repository2) => {
  ...
  const client = new OpenAI({ apiKey: config2.openAiApiKey });
...
var ensureLocalDbInitialized = async () => {
  const config2 = getConfig();
  await fs.mkdir(config2.runtimeDataDir, { recursive: true });
  await copyIfMissing(
    path2.join(config2.generatedDataDir, ENTITY_FILES.users),
    path2.join(config2.runtimeDataDir, ENTITY_FILES.users),
    []
  );
  await copyIfMissing(
    path2.join(config2.generatedDataDir, ENTITY_FILES.churches),
    path2.join(config2.runtimeDataDir, ENTITY_FILES.churches),
    []
  );
  await copyIfMissing(
    path2.join(config2.generatedDataDir, ENTITY_FILES.pastors),
    path2.join(config2.runtimeDataDir, ENTITY_FILES.pastors),
    []
  );
  await copyIfMissing(
    path2.join(config2.generatedDataDir, ENTITY_FILES.members),
    path2.join(config2.runtimeDataDir, ENTITY_FILES.members),
    []
  );
  await copyIfMissing(
    path2.join(config2.runtimeDataDir, ENTITY_FILES.insightCache),
    path2.join(config2.runtimeDataDir, ENTITY_FILES.insightCache),
    []
  );
  await copyIfMissing(
    path2.join(config2.runtimeDataDir, ENTITY_FILES.meta),
    path2.join(config2.runtimeDataDir, ENTITY_FILES.meta),
...
var loadLocalDb = async () => {
  ...
  return {
    users: await readJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.users), []),
    churches: await readJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.churches), []),
    pastors: await readJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.pastors), []),
    members: await readJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.members), []),
    insightCache: await readJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.insightCache), []),
    meta: await readJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.meta), {
      seedVersion: "v1",
...
var persistLocalDb = async (db) => {
  const config2 = getConfig();
  await writeJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.users), db.users);
  await writeJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.churches), db.churches);
  await writeJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.pastors), db.pastors);
  await writeJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.members), db.members);
  await writeJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.insightCache), db.insightCache);
  await writeJson(path2.join(config2.runtimeDataDir, ENTITY_FILES.meta), db.meta);
};
